...
public:
    ...
    // TODO
    // fix the conversion and propagation code below; seems like the right way
    // to collect rel motion is last_Tcw * curr_Twc
    //
    ...
    void predictPWLSVec(const double & dt, const size_t & num_seg_pred) {
        ...
        for (size_t i=0; i<num_seg_pred; ++i) {
            ...
            //            std::cout << tmpKine.Tcw << std::endl;

            //            std::cout << "compute full seg F" << std::endl;
            ...
            //            std::cout << "compute in seg F" << std::endl;
            ...
            //            std::cout << "propagate Xv" << std::endl;
            // update Xv
    ...
    inline bool compute_H_subblock_simplied (const arma::rowvec & Xv,
        ...
        if (check_viz) {
            ...
            if ( hrl(2,0) < 0.0 + this->mBoundDepth )
                return false;
            if(u < ORB_SLAM::Frame::mnMinX - this->mBoundXInFrame || u > ORB_SLAM::Frame::mnMaxX + this->mBoundXInFrame)
                return false;
            if(v < ORB_SLAM::Frame::mnMinY - this->mBoundYInFrame || v > ORB_SLAM::Frame::mnMaxY + this->mBoundYInFrame)
                return false;
        }
    ...
    inline void reWeightInfoMat(const Frame * F, const int & kptIdx, const MapPoint * pMP,
        ...
        // Sigma_r = Sigma_r + H_proj * H_proj.t() * std::pow(0.01, 2.0);
#ifdef OBS_DEBUG_VERBOSE
        std::cout << Sigma_r << std::endl;
#endif
    ...
    // DEBUG ONLY
    int runActiveMapMatchingWithLogging(vector<MatchingLog> & vecLog,
                                        Frame *pFrame,
                                        const size_t mat_type,
                                        const arma::mat &mBaseInfoMat,
                                        const float th,
    ...
    int runBaselineMapMatchingWithLogging(vector<MatchingLog> & vecLog, arma::mat & mCurrentInfoMat,
                                          Frame *pFrame,
                                          const size_t base_mtd,
                                          const float th,
                                          ORBmatcher &mORBMatcher,
