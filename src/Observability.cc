...
void Observability::batchInfoMat_Map(const size_t start_idx, const size_t end_idx, const double time_for_build, const bool check_viz) {
    ...
    for(size_t i = start_idx; i < end_idx; i++)  {
        ...
        if(pMP) {
            ...
            reWeightInfoMat( NULL, -1, pMP, H_meas, 0, 0, H_proj, H_rw );
            arma::mat infMat = H_rw.t() * H_rw;
            //            GoodPoint tmpLmk(static_cast<size_t>(i), 1.0, infMat);
...
bool Observability::runMatrixBuilding(const size_t mat_type, const double time_for_build,
    ...
    case ORB_SLAM::FRAME_INFO_MATRIX:
        //
        if (this->pFrame == NULL)
            return false;
    ...
    case ORB_SLAM::MAP_HYBRID_MATRIX:
        //
        if (this->mMapPoints == NULL)
            return false;
    ...
    case ORB_SLAM::FRAME_HYBRID_MATRIX:
        //
        if (this->pFrame == NULL)
            return false;
    ...
    case ORB_SLAM::FRAME_STRIP_OBS_MATRIX:
        // TODO
        break;
...
bool Observability::setSelction_Number(const size_t num_good_inlier,
    ...
    runMatrixBuilding(ORB_SLAM::FRAME_INFO_MATRIX, time_for_select/2, true);
...
bool Observability::setSelction_Number(const size_t num_good_inlier,
    ...
    //-----------------------------------------------------------------------------
    // Matrix construction for each feature (independently, therefore in parallel)
    runMatrixBuilding(ORB_SLAM::MAP_INFO_MATRIX, time_for_select/2.0, true, true);
...
int Observability::runActiveMapMatching(Frame *pFrame,
    ...
    // size_t szLazierSubset = static_cast<size_t>( float(N) / float(num_to_match) * 4.6 );
...
#ifdef OBS_DEBUG_VERBOSE
        std::cout << "func matchMapPointActive: matching round " << i << std::endl;
#endif
        ...
        while (numHit < szActualSubset) {
            ...
            if (numHit >= szActualSubset) {
                ...
                if (bestIdx >= 0) {
                    ...
                    if (mat_type == ORB_SLAM::FRAME_HYBRID_MATRIX || mat_type == ORB_SLAM::MAP_HYBRID_MATRIX) {
                        //
                        arma::mat Hyb = arma::join_vert(H_rw, H_rw * this->kinematic[this->mKineIdx].F);
                        curMat = curMat + Hyb.t() * Hyb;
                    }
                    else if (mat_type == ORB_SLAM::FRAME_INFO_MATRIX || mat_type == ORB_SLAM::MAP_INFO_MATRIX) {
                        //
...
int Observability::runActiveMapMatchingWithLogging(vector<MatchingLog> & vecLog,
                                                   Frame *pFrame,
                                                   const size_t mat_type,
                                                   const arma::mat &mBaseInfoMat,
                                                   const float th,
    ...
    // define the size of random subset
    //    double errorBound = 0.001; // 0.005; // 0.01; // 0.0005; //
    //    size_t szLazierSubset = static_cast<size_t>( 50.0 );
    //        size_t szLazierSubset = static_cast<size_t>( 6.0 );
    size_t szLazierSubset = static_cast<size_t>( float(N) / float(num_to_match) * 1.0 );
    // size_t szLazierSubset = static_cast<size_t>( float(N) / float(num_to_match) * 4.6 );
    ...
    for (size_t i=0; i<num_to_match; ++i) {
        ...
        while (numHit < szActualSubset) {
            ...
            if (numHit >= szActualSubset) {
                ...
                curCost += mMapPoints->at(heapTop.idx)->mvMatchCandidates.size();
#else
                int bestIdx = mORBMatcher.SearchByProjectionWithLogging_OnePoint(vecLog, accuMat,
                                                                                 *pFrame, mMapPoints->at(heapTop.idx), th);
#endif
                if (bestIdx >= 0) {
                    //                    std::cout << "Found match " << heapSubset.top().idx << std::endl;
                    // match succeed, move on to next batch random eval
                    ...
                    //                    vecLog.push_back(MatchingLog(ntry++, logDet( curMat )));
        ...
        if (numRndQue >= MAX_RANDOM_QUERY_TIME || heapSubset.size() == 0 || removeIdx.size() == 0) {
            std::cout << "func runActiveMapMatching: early termination!" << std::endl;
            std::cout << "numRndQue = " << numRndQue << "; heapSubset = " << heapSubset.size()  << " removeIdx = " << removeIdx.size() <<  std::endl;
            break ;
        }
...
int Observability::runBaselineMapMatching(Frame *pFrame,
    ...
    //
    if (base_mtd == ORB_SLAM::BASELINE_RANDOM) {
        // random shuffle
        std::random_shuffle ( vldPoints.begin(), vldPoints.end() );
    }
    else if (base_mtd == ORB_SLAM::BASELINE_LONGLIVE) {
        // sort by life
...
int Observability::runBaselineMapMatchingWithLogging(vector<MatchingLog> & vecLog, arma::mat & mCurrentInfoMat,
                                                     Frame *pFrame,
                                                     const size_t base_mtd,
                                                     const float th,
                                                     ORBmatcher &mORBMatcher,
    ...
    //
    size_t i;
    arma::mat accuMat = mCurrentInfoMat;
    for (i=0; i<vldPoints.size(); ++i) {
        ...
        int bestIdx = mORBMatcher.SearchByProjectionWithLogging_OnePoint(vecLog, accuMat,
                                                                         *pFrame, mMapPoints->at(vldPoints[i].idx), th);
        if (bestIdx >= 0) {
            ++ nMatched;
        }
    }
